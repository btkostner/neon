...
export const actions = {
  async login ({ commit }, { email, password }) {
    ...
    commit('login', res.data.accountLogin)
    await this.$apolloHelpers.reset()
  },
  ...
  async register ({ commit }, { name, email, password }) {
    const res = await this.$apolloHelpers.mutate({
      mutation: gql`mutation ($name: String!, $email: String!, $password: String!) {
        accountRegister(name: $name, email: $email, password: $password) {
          token
          expiredAt
          user {
            role
    ...
    commit('login', res.data.accountRegister)
    await this.$apolloHelpers.reset()
  }
}
