  ...
  def handle_event(
    ...
    case Accounts.update_user_password(socket.assigns.current_user, password, user_params) do
      {:ok, user} ->
        {:noreply,
         socket
         |> put_flash(:success, "Password Updated", "Your password has been updated successfully.")
  ...
  def handle_event(
    ...
    case Accounts.update_user_two_factor(socket.assigns.current_user, password, nil, user_params) do
      {:ok, user} ->
        {:noreply,
         socket
         |> put_flash(:success, "Two Factor Disabled", "Two factor authentication is now disabled on your account.")
  ...
  def handle_event(
    ...
    case Accounts.update_user_two_factor(socket.assigns.current_user, password, code, user_params) do
      {:ok, user} ->
        {:noreply,
         socket
         |> put_flash(:success, "Two Factor Enabled", "Two factor authentication is now enabled on your account. This will take effect next time you login.")
         |> reset_two_factor_fields(user)}
